...
|---|---|---|---|---|
| A | @ | 216.150.1.1 | Auto/300s | Required: Make mon-tech.ru direct to the site |
| CNAME | www | 4b279c13db44beaa.vercel-dns-016.com. | Auto/300s | Optional: Make www.mon-tech.ru also accessible |
...
3. Add records one by one according to the table:
   ...
   - Host Record(Name):
     - Top-level Domain(such as mon-tech.ru) use "@"(some suppliers leave empty to represent @).
     - Subdomain(such as mon-tech.ru) use prefix "@".
...
- **Subdomain Access**: If the subdomain (such as blog.mon-tech.ru) is accessed, usually only the CNAME record in the table needs to be added.
